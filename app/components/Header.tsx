...
export default function Header({
  ...
  useEffect(() => {
    ...
    if (activeTabRef.current) {
      ...
      if (tabRect.right > window.innerWidth) {
        // Scroll right to bring the tab into view
        const translateValue = tabRect.right - window.innerWidth;
        ...
        if (containerRef.current) {
          containerRef.current.scrollLeft = translateValue; // Scroll to the beginning
        }
      }
    }
  ...
  return (
    <div className="w-full h-16 sticky top-0 z-10 bg-white px-6">
      <div className="relative w-full h-full flex items-center justify-between">
        ...
        <div
          ref={containerRef}
          className="h-10 bg-white overflow-x-auto w-screen px-4 lg:h-full absolute left-1/2  -translate-x-1/2 bottom-0 translate-y-full lg:relative lg:left-0 lg:translate-x-0 xl:absolute xl:left-1/2 xl:-translate-x-1/2 xl:top-0 lg:translate-y-0 flex justify-start sm:justify-center items-center"
        >
          <ul
            id="header-list"
            className="flex items-center w-fit justify-center gap-4 font-medium"
          >
